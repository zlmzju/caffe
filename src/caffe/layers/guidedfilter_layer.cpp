...
{
    Dtype* input_data=NULL;
    const Dtype* output_data=NULL;
    switch (Caffe::mode()) 
    {
      case Caffe::CPU:
        input_data=pool_input_.mutable_cpu_data();
        caffe_copy(pool_input_.count(),src,input_data);
        pool_layer_->Forward(pool_input_vec_,pool_output_vec_);
        output_data=pool_output_.cpu_data();
        caffe_copy(pool_output_.count(),output_data,dst);
        break;
      case Caffe::GPU:
#ifndef CPU_ONLY
        input_data=pool_input_.mutable_gpu_data();
        caffe_copy(pool_input_.count(),src,input_data);
        pool_layer_->Forward(pool_input_vec_,pool_output_vec_);
        output_data=pool_output_.gpu_data();
        caffe_copy(pool_output_.count(),output_data,dst);
#else
        NO_GPU;
#endif
		...
		const vector<Blob<Dtype>*>& top)
{
    ...
    pool_param.mutable_pooling_param()->set_kernel_size(1+window_size_);
    ...
    const vector<Blob<Dtype>*>& top) 
{
  ...
  Dtype* mean_a=new Dtype[count];
  Dtype* mean_b=new Dtype[count];
  //the main loop
  for(int n=0;n<bottom[0]->num();n++)
  {
      for(int c=0;c<bottom[0]->channels();c++)	//current version channels()==1
      {
        ...
        //TODO: add private parameters mean_a and mean_b
        MeanFilter(a, mean_a);
        MeanFilter(b, mean_b);
        
        //output top
        caffe_mul(count,mean_a,I,mean_Ip);
        caffe_add(count,mean_Ip,mean_b,top_data);
      }
  }
}
...
void GuidedFilterLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) 
{
  const Dtype* I_all=bottom[0]->gpu_data();
  const Dtype* p_gpu=bottom[1]->gpu_data();
  Dtype* top_data_all=top[0]->mutable_gpu_data();
  int count=bottom[1]->count();	//make sure count=height*width
  Dtype* mean_a=new Dtype[count];
  Dtype* mean_b=new Dtype[count];
  //the main loop
  for(int n=0;n<bottom[0]->num();n++)
  {
      for(int c=0;c<bottom[0]->channels();c++)	//current version channels()==1
      {
        const Dtype* I_gpu=I_all+bottom[0]->offset(n,c);
        Dtype* top_data=top_data_all+top[0]->offset(n,c);

        Dtype* I=new Dtype[count];
        caffe_copy(count,I_gpu,I);
        Dtype* p=new Dtype[count];
        caffe_copy(count,p_gpu,p);
        ...
        //output top
        caffe_mul(count,mean_a,I,mean_Ip);
        caffe_add(count,mean_Ip,mean_b,mean_II);
        caffe_copy(count,mean_II,top_data);
      }
  }
}
